...
RSpec.describe 'BillsController', type: :request do
  ...
  it "returns the first page of bills ordered by status_last_updated descending" do
    ...
    expected_json_response_bills = JSON.parse(Bill.order(status_last_updated: :desc).limit(10).map(&:formatted_bill).to_json)
  ...
  it "returns the second page of bills ordered by status_last_updated descending" do
    ...
    expect(response).to have_http_status(:ok)
    json_response = JSON.parse(response.body)
    expected_json_response_bills = JSON.parse(Bill.order(status_last_updated: :desc).map(&:formatted_bill)[10..14].to_json)
